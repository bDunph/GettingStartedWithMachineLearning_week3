...
void ofApp::audioOut(ofSoundBuffer& output) {
    ...
    for (int i = 0; i < output.getNumFrames(); ++i) {
        ...
        /* Arpeggiator */
        currentStep = (int)metronome.phasor(4);

        if (lastStep != currentStep) {
            switch (stepCounter % 5) {
            case 0:
                arpFreq = 1.0f;
                break;
        ...
        /* FM synth */
        float modFreq = (carrierFreq * arpFreq) / ratio;
        float deviation = index * modFreq;

        double modSignal = modulator.saw(modFreq) * deviation;
        double carrierSignal = carrier.sinewave((carrierFreq * arpFreq) + modSignal) * 0.3f;
